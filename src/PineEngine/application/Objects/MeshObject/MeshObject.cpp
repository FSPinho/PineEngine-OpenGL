...
namespace PineEngine {
    GeometryBuffer &MeshObject::getGeometry() {
        return *(this->geometry);
    }

    void MeshObject::setMaterial(const Material &material_) {
        this->material = material_;
    }

    void MeshObject::performCubeMapPass(const Tick &tick, const Camera &camera, const uint32_t &cubeMapTextureId) {
        if (this->colorShader) {
            this->getTransform().moveTo(camera.getTranslation());
            this->colorShader->setUniformCubeMapTexture("CUBE", cubeMapTextureId);
            this->colorShader->setUniform("MODEL_MATRIX", this->getTransform().getMatrix());
    ...
    void MeshObject::performShadowMapPass(const Tick &tick, const Camera &lightCamera) {
        if (this->shadowMapShader) {
            this->shadowMapShader->setUniform("TIME", static_cast<float>(tick.elapsed));
            this->shadowMapShader->setUniform("MODEL_MATRIX", this->getTransform().getMatrix());
    ...
    void MeshObject::performColorPass(
        const Tick &tick,
        const Camera &camera,
    ...
    void MeshObject::performAddColorPass(const uint32_t colorTextureId, const bool multisampled) {
        if (this->colorShader) {
    ...
    void MeshObject::performPostColorPass(
    ...
    void MeshObject::markAsLightRef() {
