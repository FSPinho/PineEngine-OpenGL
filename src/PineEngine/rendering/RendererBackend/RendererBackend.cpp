...
void RendererBackend::populateDataBuffer(const uint32_t id, const void *data, const uint32_t offsetInBytes,
    ...
    this->_debugMethod(FORMAT("Populated data buffer {} with offset={} and size={}", id, offsetInBytes, sizeInBytes));
}
...
uint32_t RendererBackend::createShaders(const std::string &vertexShaderCode, const std::string &fragmentShaderCode) {
...
void RendererBackend::setUniform(const uint32_t shaderId, const std::string &name, const std::vector<float> &value) {
