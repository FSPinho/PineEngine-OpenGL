...
ShaderSet::ShaderSet(RendererBackend &backend, std::string &&vertexShaderCodePath_,
                     std::string &&fragmentShaderCodePath_)
    : RendererComponent(backend), vertexShaderCodeFile(vertexShaderCodePath_),
      vertexShaderCodeFileWatch(vertexShaderCodePath_), fragmentShaderCodeFile(fragmentShaderCodePath_),
...
void ShaderSet::_maybeReloadShaders() {
    ...
    if (!hasVertexShaderChanged && !hasFragmentShaderChanged)
        return;
    this->_loadShaders();
}
