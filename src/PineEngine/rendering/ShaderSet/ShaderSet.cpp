...
namespace PineEngine {
    ...
    void ShaderSet::setUniform(const std::string &name, const std::vector<uint32_t> &value) {
        this->intUniforms.insert_or_assign(name, value);
        this->_setUniforms();
    }
    ...
    void ShaderSet::_setUniforms() {
        if (this->areShadersLoaded) {
            for (const auto &[name, value]: this->intUniforms) {
